...
class Styles extends BaseParserClass
{
    ...
    private static function readAlignmentStyle(Alignment $alignment, \SimpleXMLElement $alignmentXml)
    {
        ...
        */
        ...
        if (isset($alignmentXml->alignment['horizontal'])) {
            $alignment->setHorizontal((string) $alignmentXml->alignment['horizontal']);
        }

        if (isset($alignmentXml->alignment['vertical'])) {
            $alignment->setVertical((string) $alignmentXml->alignment['vertical']);
        }
        ...
        */
        ...
        $textRotation = 0;
        if (isset($alignmentXml->alignment['textRotation'])) {
            $textRotationValue = (int) $alignmentXml->alignment['textRotation'];
            if ($textRotationValue <= 90) {
                $textRotation = $textRotationValue;
            } elseif ($textRotationValue > 90) {
                $textRotation = 90 - $textRotationValue;
            }
        }
        ...
        $wrapText = false;
        if (isset($alignmentXml->alignment['wrapText'])) {
            $wrapText = self::boolean((string) $alignmentXml->alignment['wrapText']);
        }

        $shrinkToFit = false;
        if (isset($alignmentXml->alignment['shrinkToFit'])) {
            $shrinkToFit = self::boolean((string) $alignmentXml->alignment['shrinkToFit']);
        }

        $indent = 0;
        if (isset($alignmentXml->alignment['indent'])) {
            $indentValue = (int) ((string) $alignmentXml->alignment['indent']);
            $indent = $indentValue > 0 ? $indentValue : 0;
        }

        $readingOrder = 0;
        if (isset($alignmentXml->alignment['readingOrder'])) {
            $readingOrderValue = (int) ((string) $alignmentXml->alignment['readingOrder']);
            $readingOrder = $readingOrderValue > 0 ? $readingOrderValue : 0;
        }

        $alignment->setWrapText($wrapText);
        $alignment->setShrinkToFit($shrinkToFit);
        $alignment->setIndent($indent);
        $alignment->setReadOrder($readingOrder);
    }
